...
class MyScript(SequentialTaskSet):
    ...
    @task
    def get_xml(self):
        res = self.client.get("/xml", name="XML")
        print(res)
    ...
    def get_robots(self):
        ...
        with self.client.get("/robots.txt", catch_response=True, name="Robots") as response:
            if expected_response in response.text:
                result = "Success"
                response.failure("Not a failure")  # Marking this success request as intentional failure
    ...
    def get_json(self):
        ...
        with self.client.get("/json", catch_response=True, name="JSON") as response:
            result = True if expected_response in response.text else False  # Pythonic way
